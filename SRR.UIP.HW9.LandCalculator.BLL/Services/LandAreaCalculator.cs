    ...
    public class LandAreaCalculator : ILandCalculator
    {
        public long CalculateLandArea(List<Point> points, bool isAltAppr = true)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                long par1 = (long) (isAltAppr ? points[i].X : points[i].Y);
                long par2_1 = (long) (isAltAppr ? points[nextIndex].Y : points[prevIndex].X);
                long par2_2 = (long) (isAltAppr ? points[prevIndex].Y : points[nextIndex].X);
